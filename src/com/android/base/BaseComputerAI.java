 ...
 * *
 *算法核心类，主体思想分三个步骤：
 *1：根据双方的当前的形势，循环地假设性的分别给自己和对方下一子（在扫描的范围内下棋），并判断此棋子能带来的形势上的变化，如能不能形成4活，或我方或敌方双3等。
 *2：根据第一次分析的结果，组合每一步棋子所带来的所有结果，装入对应的集合中，包括敌方和我方的。
 *3：根据每个AI自特色的规则，对上一步集合结果进行排序，并选取最佳的棋子。
 *
 */
...
public class BaseComputerAI extends BasePlayer{
	...
	//第二次总结果
	protected final List<SencondAnalysisResult> computerSencodResults = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> humanSencodResults = new ArrayList<SencondAnalysisResult>(10);
	
	//第二次分结果，电脑
	protected final List<SencondAnalysisResult> computer3Alive_4Half = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> computer3Alive_2Alive= new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> computer4HalfAlives = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> computerDouble3Alives = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> computer3Alives = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> computerDouble2Alives = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> computer2Alives = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> computer3HalfAlives = new ArrayList<SencondAnalysisResult>(10);
	
	//第二次分结果，人类
	protected final List<SencondAnalysisResult> human3Alive_4Half = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> human3Alive_2Alive= new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> human4HalfAlives = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> humanDouble3Alives = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> human3Alives = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> humanDouble2Alives = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> human2Alives = new ArrayList<SencondAnalysisResult>(10);
	protected final List<SencondAnalysisResult> human3HalfAlives = new ArrayList<SencondAnalysisResult>(10);
	...
	class SencondAnalysisResult implements Comparable<SencondAnalysisResult>{
		...
		Point point;			//假设的下棋位置		
		int alive4 = 0;		//活4数量				
		int halfAlive4 = 0;//半活4数量		
		int alive3 = 0; 		//活3数量		
		int halfAlive3 = 0;//半活3	
		int alive2 = 0;	    //活2数量
	...
	private void initAnalysisResults(){
		...
		//第二次分结果
		computer3Alive_4Half.clear();
		computer3Alive_2Alive.clear();
		...
		//第二次分结果，人类		
		human3Alive_4Half.clear();
		human3Alive_2Alive.clear();
	...
	private CalculateRange currentRange = new CalculateRange(0, 0, 0, 0);
	...
	private Point getFirstPoint(List<Point> humans) {
		...
		if(point.getX()==0 || point.getY()==0 || point.getX()==maxX-1 && point.getY()==maxY-1)
	...
	// 分析当前形式的入口方法，分析总共分三个步骤，第三步骤可根据AI特色进行自我配置
	...
	//一个临时对象，供第一次分析时临时存放分析结果使用，如果分析出有活2及以上的结果，则调用其cloneMe方法获得结果，否则抛弃此结果
			...
			for (FirstAnalysisResult result : list) {
				...
				if (result.count == 4) {
					...
					if (result.aliveState == ALIVE) {
						...
						if (sr.alive3 == 1 && sr.halfAlive4>0) {
							computer3Alive_4Half.add(sr);
						}
					} else {
						sr.halfAlive3++;
					...
					if (sr.alive2 == 1 && sr.alive3>0) {
						computer3Alive_2Alive.add(sr);
					}
				}
			}
	...
	private Point doThirdAnalysis() {
		...
		if(!computerDouble3Alives.isEmpty()){
			return computerDouble3Alives.get(0).point;
		}
	...
	//下棋子，对外接口
	@Override
	public void run(List<Point> humans, Point point) {
	
		//把人类下的最后一步棋子去除
		allFreePoints.remove(humans.get(humans.size()-1));
		
		//AI可以下的一步棋子
		Point result = doAnalysis(points, humans);
